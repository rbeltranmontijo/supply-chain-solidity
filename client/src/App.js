...
class App extends Component {
  ...
  listendToPaymentEvent = () => {
    //send money
    // web3.eth.sendTransaction({ to: "0x80b423e3820F6C6471f2d2D278A72361e0821B63", value: 100, from: accounts[1], gas: 300000 });
    let self = this;
    this.itemManager.events.SupplyChainStep().on("data", async function (evt) {
      console.log(evt)
      let itemObj = await self.itemManager.methods.items(evt.returnValues._itemIndex).call();
      console.log(itemObj._state)
      if (itemObj._state === "1") {
        alert(`Item id: ${evt.returnValues[0]}, identifier: ${itemObj._identifier} was paid, delivier it now`)
      }
    })
  }
